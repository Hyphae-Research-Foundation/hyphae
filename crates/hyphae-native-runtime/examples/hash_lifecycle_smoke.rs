...
//! Native whole-hash lifecycle latency smoke with durability separated.
...
const FIELD_COUNTS: [u32; 3] = [0, 64, 2_048];
...
impl TemporaryDirectory {
    fn create(label: &str) -> Result<Self, BenchmarkError> {
        ...
        let path = std::env::temp_dir().join(format!(
            "hyphae-native-hash-lifecycle-{label}-{}-{timestamp}",
            std::process::id()
        ));
        Ok(Self(path))
    }
...
fn observations() -> Result<usize, BenchmarkError> {
    match std::env::var("HYPHAE_LIFECYCLE_OBSERVATIONS") {
        Ok(value) => {
            let parsed = value.parse::<usize>()?;
            if parsed < 2 {
                return Err("HYPHAE_LIFECYCLE_OBSERVATIONS must be at least 2".into());
            }
            Ok(parsed)
        }
...
fn hash_key(label: &str, index: usize) -> Vec<u8> {
...
fn seed_hash(
    ...
    transaction.create_hash(key.to_vec())?;
    for index in 0..fields {
        let mut value = vec![u8::try_from(index % 251)?; VALUE_BYTES];
        value[..4].copy_from_slice(&index.to_le_bytes());
        transaction.hset(key.to_vec(), index.to_be_bytes().to_vec(), value)?;
    }
    Ok(())
}

fn prepare_private_database(path: &Path, fields: u32) -> Result<NativeDatabase, BenchmarkError> {
    let mut database = NativeDatabase::create(path)?;
    let mut seed = database.begin(1, DurabilityClass::Strict)?;
    seed_hash(&mut seed, b"private", fields)?;
    seed.commit()?;
    Ok(database)
}

fn measure_private_delete(fields: u32, count: usize) -> Result<Stats, BenchmarkError> {
    let temporary = TemporaryDirectory::create("private")?;
    let database = prepare_private_database(temporary.path(), fields)?;
    ...
    for logical_time in 0..count {
        ...
        if !batch.delete_hash(b"private".to_vec())? {
            return Err("private hash unexpectedly missing".into());
        }
...
fn prepare_commit_database(
    ...
    for index in 0..count {
        seed_hash(&mut seed, &hash_key(label, index), fields)?;
    }
    seed.commit()?;
    Ok(database)
}
...
fn measure_commits(
    ...
    let mut database = prepare_commit_database(temporary.path(), label, fields, count)?;
    let mut samples = Vec::with_capacity(count);
    for index in 0..count {
        let mut transaction =
            database.begin(i64::try_from(index)?.saturating_add(2), durability)?;
        if !transaction.delete_hash(hash_key(label, index))? {
            return Err("commit hash unexpectedly missing".into());
        }
        let started = Instant::now();
        transaction.commit()?;
        samples.push(nanos(started.elapsed())?);
...
fn observe(fields: u32, count: usize) -> Result<Observation, BenchmarkError> {
    Ok(Observation {
        fields,
        private_delete: measure_private_delete(fields, count)?,
        memory_commit: measure_commits(fields, count, DurabilityClass::Memory, "memory")?,
        strict_commit: measure_commits(fields, count, DurabilityClass::Strict, "strict")?,
    })
}
...
fn print_report(count: usize, results: &[Observation]) {
    println!("{{");
    println!("  \"schema\": \"hyphae-native-hash-lifecycle-smoke-v1\",");
    ...
    for (index, result) in results.iter().enumerate() {
        ...
        print_stats("private_delete_hash", result.private_delete, true);
...
fn main() -> Result<(), BenchmarkError> {
    ...
    let results = FIELD_COUNTS
        .into_iter()
        .map(|fields| observe(fields, count))
