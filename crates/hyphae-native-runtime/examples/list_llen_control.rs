...
//! Matched persistent-list LLEN control for whole-list TTL changes.
...
impl TemporaryDirectory {
    fn create() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        Ok(Self(std::env::temp_dir().join(format!(
            "hyphae-native-list-llen-control-{}-{timestamp}",
            std::process::id()
        ))))
    }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if database.llen_latest_list(LIST_KEY)? != usize::try_from(LIST_ELEMENTS)? {
        return Err("control list length does not match the seeded element count".into());
    }
    for _ in 0..WARMUP {
        black_box(database.llen_latest_list(LIST_KEY)?);
    }
    ...
    println!("{{");
    println!("  \"schema\": \"hyphae.native.list-llen-control.v1\",");
