# ADR-0025: GPLv3 code and CC BY-SA documentation
...
while requiring distributed software derivatives to remain available under
the same software license. Documentation needs equivalent attribution and
share-alike terms without applying a documentation license to executable code.
...
Hyphae software uses `GPL-3.0-only`. Software includes Rust, Python,
...
The root `LICENSE` contains the canonical GPLv3 text,
...
- Distributed modified software must satisfy GPLv3 source and license duties.
- GPLv3 does not add a network-use source obligation; operating a modified
  service without conveying the software is outside that trigger.
...
contents, and absence of stale Apache-2.0 project declarations. Historical
