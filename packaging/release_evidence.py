...
def source_identity(commit: str) -> ReleaseIdentity:
    ...
    return ReleaseIdentity(
        ...
        version=version,
        tag=f"release-v{version}-final-evidence",
    )
...
def expected_primary_names(
    ...
    *,
    ...
    expected = (
        ...
        | {
            f"hyphae-release-v{version}-final-evidence.spdx.json",
            f"hyphae-release-v{version}-final-evidence.cdx.json",
        }
    )
    if include_required_checks:
        expected.add(required_checks_name(f"release-v{version}-final-evidence"))
    return expected
