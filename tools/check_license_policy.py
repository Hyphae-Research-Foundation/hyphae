...
def validate_spdx_file(path: Path) -> str | None:
    ...
    expected = f"SPDX-License-Identifier: {GPL_IDENTIFIER}"
...
def validate_openapi_license(path: Path) -> str | None:
    ...
    expected = re.compile(
        rf"(?m)^  license:\n    name: {re.escape(GPL_IDENTIFIER)}\n"
        rf"    identifier: {re.escape(GPL_IDENTIFIER)}$"
    )
    if expected.search(encoded) is None:
        return f"{path}: OpenAPI license is not {GPL_IDENTIFIER}"
    return None
...
def validate_repository(root: Path = ROOT) -> list[str]:
    ...
    if toml_string(root / "Cargo.toml", "workspace.package", "license") != GPL_IDENTIFIER:
        failures.append("Cargo.toml: workspace license differs from GPL-3.0-only")
    if toml_string(root / "fuzz" / "Cargo.toml", "package", "license") != GPL_IDENTIFIER:
        failures.append("fuzz/Cargo.toml: package license differs from GPL-3.0-only")
    if (
        toml_string(
        ...
        failures.append("sdks/python/pyproject.toml: license differs from GPL-3.0-only")
    ...
    for relative, keys in json_licenses:
        ...
        for key in keys:
            if not isinstance(value, dict) or key not in value:
                ...
                break
            value = value[key]
        if value != GPL_IDENTIFIER:
            failures.append(f"{relative}:{'.'.join(keys)} differs from {GPL_IDENTIFIER}")
    ...
    for marker in (
        f"// SPDX-License-Identifier: {GPL_IDENTIFIER}",
        f"# SPDX-License-Identifier: {GPL_IDENTIFIER}",
    ):
    ...
    for identifier in (GPL_IDENTIFIER, DOCUMENTATION_IDENTIFIER):
    ...
    if f'"{GPL_IDENTIFIER}"' not in deny_policy:
        failures.append("deny.toml: workspace GPL-3.0-only license is not allowed")
    return failures
...
def main() -> int:
    ...
    if failures:
        ...
        return 1
    print(
        "license policy passed: canonical GPL-3.0-only code, "
