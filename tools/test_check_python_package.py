...
class PythonPackageContractTests(unittest.TestCase):
    ...
    def test_checked_in_package_is_publishable(self) -> None:
        self.assertEqual(
            validate(),
            {"name": "hyphae-sdk", "status": "passed", "version": "1.2.1"},
        )

    def test_python_publish_rejects_apache_1_1_0(self) -> None:
        with self.fixture() as directory:
            root = Path(directory)
            self.remove_from_workflow(root, 'test "$version" = "1.2.1"')
            with self.assertRaisesRegex(PythonPackageValidationError, "workflow|1.2.1"):
                validate(root)
    ...
    def test_python_version_must_equal_the_workspace_version(self) -> None:
        with self.fixture() as directory:
            root = Path(directory)
            self.replace(root, 'version = "1.2.1"', 'version = "1.2.2"')
