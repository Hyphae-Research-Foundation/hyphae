...
class PythonPackageContractTests(unittest.TestCase):
    ...
    def test_checked_in_package_is_publishable(self) -> None:
        self.assertEqual(
            validate(),
            {"name": "hyphae-sdk", "status": "passed", "version": "2.0.0"},
        )

    def test_python_publish_rejects_apache_1_1_0(self) -> None:
        with self.fixture() as directory:
            root = Path(directory)
            self.remove_from_workflow(root, 'test "$version" = "2.0.0"')
            with self.assertRaisesRegex(PythonPackageValidationError, "workflow|2.0.0"):
                validate(root)
    ...
    def test_python_version_must_equal_the_workspace_version(self) -> None:
        with self.fixture() as directory:
            root = Path(directory)
            self.replace(root, 'version = "2.0.0"', 'version = "1.2.3"')
