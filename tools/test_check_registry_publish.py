...
class RegistryPublishGateTests(unittest.TestCase):
    def test_crate_download_requests_the_archive_not_a_url_document(self) -> None:
        ...
        with patch("tools.check_registry_publish.urllib.request.urlopen", opener):
            _url_bytes("https://crates.io/api/v1/crates/demo/2.1.0", "metadata")
            _url_bytes(
                "https://crates.io/api/v1/crates/demo/2.1.0/download",
                "crate",
                accept=None,
            )
    ...
    def test_crate_vcs_metadata_accepts_clean_and_rejects_dirty_sources(self) -> None:
        ...
        def crate(vcs: dict) -> bytes:
            ...
            with tarfile.open(fileobj=payload, mode="w:gz") as archive:
                ...
                member = tarfile.TarInfo("demo-2.1.0/.cargo_vcs_info.json")
        ...
        self.assertEqual(_crate_vcs_commit(crate(clean_without_marker), "demo", "2.1.0"), COMMIT)
        self.assertEqual(_crate_vcs_commit(crate(clean_with_marker), "demo", "2.1.0"), COMMIT)
        with self.assertRaisesRegex(GateFailure, "dirty"):
            _crate_vcs_commit(crate(dirty), "demo", "2.1.0")
